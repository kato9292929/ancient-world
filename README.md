...
- **`aw-data`** — スキーマ・検証・座標取得スクリプト・フィクスチャ単体テスト。`data/*.json` は台帳の投入待ち（空配列）
- **`aw-timeline`** — 縦スクロール年表。座標を使わないため、座標取得を待たず完成できる。尺度は線形固定（案A）。JS 無効でも全項目が読める

残り（`aw-map` / `aw-objects` / `aw-compare` / `aw-lp`）は順次追加する。
